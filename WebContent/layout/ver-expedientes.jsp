...
<body>
                        ...
                        <div class="page-title-wrapper">
                            ...
                            <div class="page-title-heading">
                                <div class="page-title-icon">
                                    <i class="fas fa-folder icon-gradient bg-mean-fruit">
                                    </i>
                                </div>
                                <div>Vista de expedientes
                                    <div class="page-title-subheading">Vista de los expedientes
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="card-body">
                                    <table id="example1" class="table table-bordered table-striped">
                                        ...
                                        <tbody>
                                        	...
                                        	DTVExpedientes dte = new DTVExpedientes();
                                        	ArrayList<VW_Expedientes> listarExpedientes = new ArrayList<VW_Expedientes>();
                                        	listarExpedientes = dte.listarExpediente();
                                        	int crear = 1;
											for (VW_Expedientes ve: listarExpedientes){
											%>
                                            <tr>
                                            	<td><%=ve.getAsignatura()%></td>
                                            	<td><%=ve.getEdicion()%></td>
                                            	<td>
                                            	<%
														Syllabus s = dts.buscarSyllabusporExp(ve.getId_expediente_asignatura());
														if(s.getId_syllabus() == 0){
												%>
                                            	<span title="Crear Syllabus"> 
                                            	<a onClick="crearSyllabus('<%=ve.getId_expediente_asignatura()%>','<%=crear%>')"> 
                                            		<i class="fas fa-scroll" style="color:blue;"></i>
												</a>
												</span> 
												<%} %>
												<%
														if(s.getId_syllabus() != 0){
												%>
												<span title="Ver Syllabus">
												<a 
												href="ver-syllabus.jsp?idexpediente=<%=ve.getId_expediente_asignatura()%>&idsyllabus=<%=s.getId_syllabus()%>">
  ...
  <!-- inicializar tabla -->
     ...
     <script>
		function crearSyllabus(id_expediente, opcion)
		{
			window.location.href="../SLSyllabus?id_expediente="+id_expediente+"&opcionSyllabus="+opcion;
			location.reload
